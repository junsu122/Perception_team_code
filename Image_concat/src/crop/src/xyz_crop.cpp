    ...
    void velodyneCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        for (const auto &field : msg->fields) {
            if (field.name == "x") {
                ...
                z_offset = field.offset; // z 값의 offset 찾기
            }
        }
        
        std::cout << "offset_xyz : " << x_offset << " " << y_offset << " " << z_offset << std::endl;
        ...
        const float max_y = 3.0;
        const float min_z = -4.0;  // z 값 최소 범위 (필터링 범위 설정)
        const float max_z = 0.8;  // z 값 최대 범위 (필터링 범위 설정)
        ...
        for (size_t i = 0; i < msg->data.size(); i += msg->point_step)
        {
            ...
            std::memcpy(&z, &msg->data[i + z_offset], float_size); // z 값 읽기

            if (x >= min_x && x <= max_x && y >= min_y && y <= max_y && z >= min_z && z <= max_z) // z 값 필터링 추가
            {
