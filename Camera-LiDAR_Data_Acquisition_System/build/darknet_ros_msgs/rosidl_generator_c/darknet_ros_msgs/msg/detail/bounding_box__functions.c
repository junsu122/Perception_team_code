  ...
  darknet_ros_msgs__msg__BoundingBox__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    darknet_ros_msgs__msg__BoundingBox * data =
      (darknet_ros_msgs__msg__BoundingBox *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!darknet_ros_msgs__msg__BoundingBox__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          darknet_ros_msgs__msg__BoundingBox__fini(&output->data[i]);
        }
        return false;
      }
    }
